...
ROLE_STYLE = {   # 역할 → (배경, 글자색)  · 명찰은 한 종류로 찍고 배지 색으로만 구분
    "참가자": ("linear-gradient(135deg,#ecd69f,#c9a86a)", "#2c1342"),
    "강사":   ("linear-gradient(135deg,#8f5fd0,#6533b0)", "#ffffff"),
    "섬김이": ("transparent", "#7b4fc0"),
    "운영":   ("transparent", "#7b4fc0"),
    "스태프": ("transparent", "#7b4fc0"),
}
...
def css(font_prefix: str) -> str:
  ...
  * {{ box-sizing:border-box; }}
  ...
  .badge {{ position:relative; overflow:hidden; color:#33174f;
    ...
    background:
      radial-gradient(ellipse 120% 70% at 50% -6%, rgba(155,98,221,.22), transparent 62%),
      linear-gradient(180deg,#f3ecfc 0%,#faf7fe 55%,#ffffff 100%); }}
  .deco {{ position:absolute; inset:0; z-index:0;
    background-image:
      linear-gradient(rgba(123,79,192,.16) .28cqw, transparent .28cqw),
      linear-gradient(90deg, rgba(123,79,192,.16) .28cqw, transparent .28cqw);
  ...
  .card {{ position:absolute; inset:4.4cqw; z-index:2; border-radius:3.4cqw;
    ...
    background:linear-gradient(180deg,#fff 0%,#fbf8fe 62%,#f6f0fc 100%);
  ...
  .header {{ position:relative; text-align:center; color:#fff;
    background:linear-gradient(132deg,#5f34a6 0%,#8a52d2 52%,#bb8cec 100%);
  ...
  .name {{ font-weight:900; letter-spacing:-.05em; line-height:1; white-space:nowrap; color:#2e1348; }}
  .belong {{ font-size:4cqw; color:#6b5b8a; letter-spacing:-.01em; }}
  ...
  .day {{ background:rgba(123,79,192,.052); border-radius:2.4cqw; padding:1cqw 1.6cqw; }}
  ...
  .pill {{ display:inline-flex; align-items:baseline; gap:1.2cqw; white-space:nowrap;
    background:linear-gradient(120deg,#6f3fb5,#9c62dd); color:#fff;
  ...
  .no {{ width:4.4cqw; height:4.4cqw; border-radius:50%; background:#fff;
    border:.22cqw solid rgba(123,79,192,.3); color:#6f3fb5; font-size:2.75cqw;
    font-weight:800; display:flex; align-items:center; justify-content:center; line-height:1; }}
  .t {{ font-size:3.5cqw; font-weight:700; color:#7b4fc0;
    font-variant-numeric:tabular-nums; letter-spacing:-.03em; }}
  .s {{ font-size:3.8cqw; font-weight:600; color:#241038; line-height:1.1; letter-spacing:-.03em; }}
...
def front_card(ev: dict, person: dict) -> str:
    role = (person.get("역할") or person.get("role") or "").strip()
    bg, fg = ROLE_STYLE.get(role, ("transparent", "#7b4fc0"))
    border = "" if bg != "transparent" else "border:.42cqw solid rgba(123,79,192,.55);"
...
def page_doc(title: str, style_extra: str, cards: list[str], font_prefix: str) -> str:
    return f"""<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8">
<title>{esc(title)}</title><style>{css(font_prefix)}{style_extra}</style></head>
...
"""
    cards = [front_card(ev, p) for p in people]
    f1 = outdir / "_front.html"
    f1.write_text(page_doc("명찰 앞면", one, cards, ""), encoding="utf-8")
    pdf1 = html_to_pdf(f1, outdir / "badges_print.pdf")
    ok(f"badges_print.pdf   {len(people)}명 · 한 장에 한 명 ({w:.0f}×{h:.0f}mm)")

    # ② 뒷면 일정표 (공통 1장)
    f2 = outdir / "_back.html"
    f2.write_text(page_doc("명찰 뒷면", one, [back_card(ev)], ""), encoding="utf-8")
...
"""
        f3 = outdir / "_a4.html"
        f3.write_text(page_doc("명찰 A4 배치", style, pages, ""), encoding="utf-8")
