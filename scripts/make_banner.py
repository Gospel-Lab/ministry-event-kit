...
def build(ev: dict, outdir: Path) -> None:
    ...
    # ── 세로 배치 ──
    cy = bleed + trim_h / 2
    block_h = org_size * 0.9 + t_size * 0.86 + (sub_size * 1.6 if subtitle else 0)
    top = cy - block_h / 2
    org_baseline = round(top + org_size * 0.75, 1)
    t_base = round(org_baseline + t_size * 0.92, 1)
    sub_baseline = round(t_base + sub_size * 2.0, 1)
    ...
    ok(f"인쇄용 PDF  {pdf.name}  ({trim_w:.0f} × {trim_h:.0f} mm)")

    jpg = pdf_to_cmyk_image(pdf, outdir / "banner_CMYK_150dpi.jpg", dpi=150)
    if jpg:
        mb = jpg.stat().st_size / 1048576
        ok(f"CMYK 이미지 {jpg.name}  ({mb:.1f} MB, 150dpi)")
...
작업 규격 : {full_w:.0f} × {full_h:.0f} mm (도련 사방 {bleed:.0f}mm 포함)
마감 방식 : {finish}
